...
function createScoreboard(objectiveName, displayName) {
    execCommand(
        'scoreboard objectives add ' + objectiveName + ' dummy ' + displayName
    );
...
function addPlayerToTeam(objectiveName, teamName, playerName) {
    ...
    execCommand(
        'scoreboard players set ' + playerName + ' ' + objectiveName + ' -1'
    );
...
function updatePlayerScore(objectiveName, playerName, score) {
    execCommand(
        'scoreboard players set ' + playerName + ' ' + objectiveName + ' ' + score
    );
}
